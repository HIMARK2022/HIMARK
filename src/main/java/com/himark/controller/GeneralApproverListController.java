...
public class GeneralApproverListController {
	...
	public void tree(Model model , @RequestParam("userId")String userId) {
		...
		String depart_name = service.getDepartL(userId).get(0).getDept_name().toString();
		String upper_name = service.getUpperL(userId).get(0).getDept_name().toString();
		...
		if(service.tempManagerT(team_name).size() != 0) {
			String tid = service.tempManagerT(team_name).get(0).getUser_id().toString();
			service2.searchUserById(tid);
			model.addAttribute("user_team",service2.searchUserById(tid));
		}
		if(service.tempManagerD(depart_name).size() != 0) {
			String did = service.tempManagerD(depart_name).get(0).getUser_id().toString();
			service2.searchUserById(did);
			model.addAttribute("user_dept",service2.searchUserById(did));
		}
		if(service.tempManagerH(upper_name).size() != 0) {
			String uid = service.tempManagerH(upper_name).get(0).getUser_id().toString();
